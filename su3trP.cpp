...
int metropolis_trP(gauge_field &U, mdp_matrix_field &S, mdp_matrix_field &Pls,
		   mdp_matrix_field &T1, mdp_matrix_field &T2, float beta, float h) {
  ...
  for(int parity=1; parity>=0; parity--)
     ...
     staple_plaq = load_staples(U, S, mu, parity);
	...
	oldaction=(0.333333*beta)*real(trace( U(x,mu) * S(x) ));
	newaction=(0.333333*beta)*real(trace( newU * S(x) ));
  ...
  for(int parity=1; parity>=0; parity--) {
	...
	//	zP += ploop_less_slice_t(U, Pls, mu, t, T1, T2, opp_parity);
...
int main(int argc, char** argv) {
  ...
  **/
  ...
  zP = ave_polyakov_loop(U, 3, T1, T2, T3);
  if(ME==0) {
     cout << "INIT "<< beta <<" "<< h <<" "<< plaq <<" "
	  << zP.real() <<" "<< zP.imag() <<" "<< abs(zP) <<endl;
  }
  ...
  for(i=0; i<warms; i++) {
     metropolis_trP(U, S, Pls, T1, T2, beta, h);
  }
